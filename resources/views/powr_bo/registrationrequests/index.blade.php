...
@section('contents')
		...
		<div class="container-fluid clearfix">
			<div class="row">
              ...
              <h2 class="h3 main-heading">Списък на предприятията, които са подали заявка за регистрация като такива, които  осигуряват временна работа, съгласно Закон за насърчаване на заетостта (изм. ДВ, бр. 7 от 24.01.2012г.)</h2>

							<div class="form-items">
									<section>
											<!--<h3><span class="primary-bgr">Списък регистрации</span></h3>-->
                      <h3><span class="primary-bgr">Заявки за регистрации на ПОВР</span></h3>
												<table class="table">
												<thead>
                          <tr>
                            ...
                            <th scope="col">ЗЗЛДС чл. 10, ал. 1, т. 2 номер</th>
                            <th scope="col">Предприятие тип</th>
                          </tr>
												</thead>
												<tbody>
                          @foreach ($powr_regrequests as $row)

									<tr>
                              <td>
                                <a href="{{route('powr_bo.registrationrequests.edit', $row->maxPrototypeId)}}" class="text-primary">{{ $row->maxPrototypeId }}</a>
			      </td>
  <td>{{ $row->name }} </td>
  <td>{{ $row->uic }}</td>
			      <td>{{ $row->zzldcode }}</td>
