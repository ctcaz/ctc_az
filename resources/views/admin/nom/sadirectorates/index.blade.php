<!--
		File:	resources\views\admin\nom\sadirectorates\index.blade.php
		 Ver:	1.00.003
 Purpose:	SA Directorate index blade
...
@section('contents')
        ...
        <div class="content">
        	...
        	<h2 class="h3 main-heading">Списък с бюра по труда</h2>
					<p><a href="{{route('nom.sadirectorates.create')}}" class="btn btn-outline btn-outline-primary btn-lg">Добави бюро по труда</a></p>
					...
					<div class="form-items">
						<section>
								<h3><span class="primary-bgr">Критерии за търсене</span></h3>
										...
										<h6><strong>Списък с бюра  по труда</strong></h6>
										...
										<tbody>
											@foreach ($n_sadirectorate as $n_sadirectorates)
												<tr>
													<td>
														<a href="{{route('nom.sadirectorates.edit', $n_sadirectorates->id)}}" class="text-primary">{{ $n_sadirectorates->name }}</a>
													</td>

													<td>{{ $n_sadirectorates->code }}</td>

													<td>{{ $n_sadirectorates->is_active }}</td>
												</tr>
											@endforeach
										</tbody>
									</table>

									{!! $n_sadirectorate->links() !!}
