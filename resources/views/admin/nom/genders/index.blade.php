...
@section('contents')
        ...
        <div class="content">
        	...
        	<h2 class="h3 main-heading">Списък полове</h2>
					<p><a href="{{route('nom.genders.create')}}" class="btn btn-outline btn-outline-primary btn-lg">Добави пол</a></p>
					...
					<div class="form-items">
						<section>
								<h3><span class="primary-bgr">Критерии за търсене</span></h3>
										...
										<h6><strong>Списък полове</strong></h6>
										...
										<tbody>
											@foreach ($n_gender as $ngender)
												<tr>
													<td>
														<a href="{{route('nom.genders.edit', $ngender)}}" class="text-primary">{{ $ngender->name }}</a>
													</td>

													<td>{{ $ngender->code }}</td>

													<td>{{ $ngender->is_active }}</td>
												</tr>
											@endforeach
										</tbody>
									</table>
