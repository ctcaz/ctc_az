<?php /*<!--
		File:	resources\views\admin\nom\workregimes\index.blade.php
		 Ver:	1.00.003
 Purpose:	Work regime index blade
...
@section('contents')
        ...
        <div class="content">
        	...
        	<h2 class="h3 main-heading">Списък с видове работно време</h2>
					<p><a href="{{route('nom.workregimes.create')}}" class="btn btn-outline btn-outline-primary btn-lg">Добави вид работно време</a></p>
						...
						<section>
								<h3><span class="primary-bgr">Критерии за търсене</span></h3>
										<div class="row">
											...
											<div class="col">
												<label for="">Код:</label>
												<input type="text" class="form-control" id="fi_old_code">
											</div>
										</div>
										...
										<h6><strong>Списък с видове работно време</strong></h6>
										...
										<tbody>
											@foreach ($n_workregime as $n_workregimes)
												<tr>
													<td>
														<a href="{{route('nom.workregimes.edit', $n_workregimes->id)}}" class="text-primary">{{ $n_workregimes->name }}</a>
													</td>

													<td>{{ $n_workregimes->old_code }}</td>

													<td>{{ $n_workregimes->is_active }}</td>
												</tr>
											@endforeach
										</tbody>
									</table>
