...
@section('contents')
        ...
        <div class="content">
        	...
        	<h2 class="h3 main-heading">Списък области</h2>
					<p><a href="{{route('regions.create')}}" class="btn btn-outline btn-outline-primary btn-lg">Добави област</a></p>
					...
					<div class="form-items">
						<section>
								<h3><span class="primary-bgr">Критерии за търсене</span></h3>
										...
										<tbody>
											@foreach ($n_regions as $n_region)
												<tr>
													<td style="display:none;">{{ $n_region->id }}</td>
													<td>
														<a href="{{route('regions.edit', $n_region->id)}}" class="text-primary">{{ $n_region->name }}</a>
													</td>

													<td>{{ $n_region->code }}</td>

													<td>{{ $n_region->is_active }}</td>
												</tr>
											@endforeach
										</tbody>
									</table>
