<!--
		File:	resources\views\admin\nom\prefcontrtypes\index.blade.php
		 Ver:	1.00.003
...
@section('contents')
        ...
        <div class="content">
        	...
        	<h2 class="h3 main-heading">Списък договори</h2>
					<p><a href="{{route('nom.prefcontrtypes.create')}}" class="btn btn-outline btn-outline-primary btn-lg">Добави договор</a></p>
					...
					<div class="form-items">
						<section>
								<h3><span class="primary-bgr">Критерии за търсене</span></h3>
										...
										<h6><strong>Списък договори</strong></h6>
										...
										<tbody>
											@foreach ($n_preffered_contract_type as $nprefcontrtypes)
												<tr>
													<td>
														<a href="{{route('nom.prefcontrtypes.edit', $nprefcontrtypes)}}" class="text-primary">{{ $nprefcontrtypes->name }}</a>
													</td>

													<td>{{ $nprefcontrtypes->code }}</td>

													<td>{{ $nprefcontrtypes->is_active }}</td>
												</tr>
											@endforeach
										</tbody>
									</table>

									{!! $n_preffered_contract_type->links() !!}
